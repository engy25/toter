...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form method="post" action="{{ route('allusers.update', ['alluser' => $alluser->id]) }}" enctype="multipart/form-data">
      @csrf
      ...
      <div class="mb-3">
        ...
        <input type="text" value="{{ $alluser->fname }}" class="form-control" id="fname" name="fname" required>
        @error('fname')
      ...
      <div class="mb-3">
        ...
        <input type="text" value="{{ $alluser->lname  }}" class="form-control" id="lname" name="lname" >
        @error('lname')
        <div class="alert alert-danger" role="alert">
          {{ $message }}
        </div>
      ...
      <div class="mb-3">
        ...
        <input type="email" value="{{ $alluser->email  }}" class="form-control" name="email" required>
      </div>
        ...
        <div class="input-group">
          <select class="form-control" id="country" name="country" style="max-width: 70px;">
            ...
            <option value="{{ $country->country_code }}" {{ $country->country_code ==
              $alluser->country_code ? 'selected' : '' }}>{{ $country->country_code }}</option>
            @endforeach
          </select>
          <input type="text" class="form-control" id="phone" value="{{ $alluser->phone  }}" name="phone" required>
        </div>
      </div>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-10">
          ...
          @if($alluser->image)
          <div>
            <img id="image-preview" src="{{ asset($alluser->image) }}" alt="Store Image" style="max-width: 200px;">
          </div>
          @else
          <div>
