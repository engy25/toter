...
<div class="text-center mb-3">
  ...
  <a href="{{ route('addons.create') }}" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addAddonModal">Add New Addon </a>
</div>
    ...
    <tbody>
      @forelse ($addons as $addon)
        <tr>
          <td>{{ $addon->name }}</td>
          <td>
            <img src="{{ asset($addon->image) }}" alt="Ingredient Image" style="height: 20%; width:20%"
              class="img-fluid">
          </td>
          <td>{{ $addon->price }} {{ $item->currencyIsoCode }}</td>
