...
<script>
  $(document).ready(function(){
    ...
    let name_en = $('#the_name_en').val();
    let name_ar= $('#the_name_ar').val();
    let add=$('#add').val();
    ...
    $.ajax({
      ...
      url: "{{ route('ingredientpoints.store') }}",
      method: 'post',
      data: formData,
      dataType: "json",
...
$(document).on('click', '.delete-ingredient', function (e) {
    ...
    if (confirm("Are you sure you want to delete this Ingredient?")) {
        $.ajax({
            url: '{{ url("ingredientpoints") }}/' + ingredient_id,
            type: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}',
                 ingredientpoint: ingredient_id
            },
            success: function (data) {
              console.log(data);
                if (data.status == true) {
