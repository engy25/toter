  ...
  <tbody>
    @forelse($items as $item)
      <tr>
        ...
        <td><span class="text-dark fs-13 fw-semibold">{{ $item->category->name }}</span></td>

        <td><span class="text-dark fs-13 fw-semibold">{{ $item->price }}</span></td>
        ...
        <td class="center align-middle">
          <div class="btn-group">
            <a href="{{ route('itempoints.edit', $item->id) }}" class="btn bg-info-transparent d-flex align-items-center justify-content-center">
              <i style="font-size: 20px;" class="fe fe-edit text-info "></i>
            </a>

            <a href="{{ LaravelLocalization::localizeURL(route('itempoints.edit', $item->id)) }}" class="btn btn-info btn-icon py-1 me-2"
               data-id="{{ $item->id }}"
            ...
            <a href="{{ route('itempoints.show', $item->id) }}" class="btn btn-success show-item" style="width: 100px; height: 40px;">
    ...
    @empty
      <tr>
        <td colspan="7" class="text-center">No Items found for this store.</td>
      </tr>
    @endforelse
  </tbody>
</table>
