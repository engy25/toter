    ...
    <tbody>
      @forelse ($ingredients as $ingredient)
        <tr>
          ...
          <td>{{ $ingredient->price }} {{ $store->currencyIsoCode }}</td>
          ...
          <td class="center align-middle">
            <div class="btn-group">
              ...
              <a href="{{ LaravelLocalization::localizeURL(route('ingredients.edit', $ingredient->id)) }}"
                ...
                data-bs-target="#updateModal" data-id="{{ $ingredient->id }}"
                {{-- data-name_en="{{ $ingredient->translations()->where("locale","en")->first()->name }}"
                data-name_ar="{{$ingredient->translations()->where("locale","ar")->first()->name }}" --}}
                data-image="{{ $ingredient->image }}" title="Edit"
