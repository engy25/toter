<table id="data-table2" class="table border p-0 text-nowrap mb-0">
  <thead class="tabel-row-heading text-dark">
    <tr style="background:#f4f5f7">
      ...
      <th class="fw-semibold border-bottom">Action</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        @if($order->driver)
        <span class="text-dark fs-13 fw-semibold">{{ $order->delivery->fname }} </span>
        @else
      ...
      <td class="center align-middle">
        <div class="btn-group">
          <a href="{{ route('storeorderscallcenters.show', ['storeorderscallcenter' => $order->id]) }}" class="btn btn-success" title="Order Details">
            Show Order
          </a>&nbsp;

          @if($order->deliveryTrack )
            <a href="{{ route('create.track.ordercallcenter',['order'=>$order->id]) }}" class="btn btn-primary" title="Track Order">
              Track Order
            </a>
            @endif
