  ...
  <div class="card-body">
    ...
    <form method="post" action="{{ route('orders.store', ['id' => $id->id]) }}" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <select class="form-control" id="tagFilter" name="tag_id" onchange="filterItems()">
      ...
      <div class="row mb-4" id="filteredItemsContainer">
        <!-- Display Item Card -->
        @foreach($store->Theitems as $item)
        <div class="col-md-5 col-lg-3 mb-2">
          <div class="card h-100">
            ...
            <a href="{{ route("item.details",$item->id) }}"> <img class="img-fluid" src="{{ $item->image }}"
                alt="Item Image" /></a>
            <div class="card-body">
              <p class="card-text">{{ $item->price }} {{ $item->currencyIsoCode }}</p>
              ...
              <button type="button" class="btn btn-success"
                onclick="addItem({{ $item->id }}, {{ $item->price }})">Add</button>
            </div>
          </div>
        </div>
        @endforeach
...
<script>
  function filterItems() {
    ...
    var selectedTagId = document.getElementById('tagFilter').value;

    $.ajax({
  url: '/filter-items/' + selectedTagId,
  type: 'GET',
  success: function (data) {
    console.log(data);

    // Update the HTML content with the new items
    var itemsContainer = document.getElementById('filteredItemsContainer');
    ...
    // Display the filtered items
      ...
      data[0].forEach(function (item) {
        var newItemHtml = '<div class="col-md-5 col-lg-3 mb-2">' +
          '<div class="card h-100">' +
          '<div class="card-body">' +
          '<h5 class="card-title">' + item.name + '</h5>' +
          '<h6 class="card-subtitle text-muted">' + item.description + '</h6>' +
          ...
          '<p class="card-text">' + item.price + ' ' + data[1] + '</p>'+

          '<div class="form-group">' +
          '<label for="quantity' + item.id + '">Quantity:</label>' +
          '<input type="number" class="form-control" id="quantity' + item.id + '" name="quantities[' + item.id + ']" value="1" min="1">' +
          '</div>' +
          '<br>'+
          '<button type="button" class="btn btn-success" onclick="addItem(' + item.id + ', ' + item.price + ')">Add</button>'
          '<br>' +
          '</div>' +
          '</div>' +
          '</div>';
...
<script>
  function addItem(itemId,itemPrice) {
      ...
      var itemObject = {
          id: itemId,
          quantity: quantity,
          // price: itemPrice, // Replace this with the actual price of the item
